...
### Database schema
SQLAlchemy ORM models in `backend/app/models` are the schema source of truth.

After any model change, generate the Alembic revision with `alembic revision --autogenerate`. Do not create or edit migration files by hand to describe tables, columns, indexes, constraints, or types. Data backfills may be added to an autogenerated revision. `alembic check` must pass.
...
- Database migrations are autogenerated from the ORM, apply cleanly, and `alembic check` passes.
